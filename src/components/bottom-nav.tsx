...
function DockItem({
  ...
  const size = useSpring(
    useTransform(distance, [-MAGNIFY_RADIUS, 0, MAGNIFY_RADIUS], [BASE_SIZE, MAX_SIZE, BASE_SIZE]),
    SPRING,
  );
    ...
    <Link href={href} aria-label={label} aria-current={active ? "page" : undefined}>
      <motion.div
        ...
        className={cn(
          ...
          { "shadow-[2px_2px_20px_0px_rgba(32,157,113,0.5)] border-primary-950": active },
        )}
      >
        <motion.div style={{ scale }}>
          <Icon
            className={cn("h-5 w-5", active ? "text-primary-500" : "text-foreground-muted")}
            strokeWidth={1.75}
          />
        </motion.div>
      </motion.div>
  ...
  return (
    ...
    >
      <div
        ...
        style={{ height: BASE_SIZE + PADDING * 2, paddingInline: PADDING, paddingBottom: PADDING }}
      >
        ...
        {ITEMS.map((item) => (
          <DockItem key={item.href} mouseX={mouseX} active={pathname === item.href} {...item} />
        ))}
      </div>
    </nav>
  );
